...
class BaPengawasanBongkarController extends Controller
{
    ...
    {
        $bongkar = TblBaPengawasanBongkar::select('id', 'tgl_pendaftaran_ba_bongkar', 'no_pendaftaran_ba_bongkar')->get();

        // dd($bongkar);

        $bongkar = $bongkar->map(function ($item) {
            $item->tgl_pendaftaran_ba_bongkar = $this->formatDates(['tgl_pendaftaran_ba_bongkar' => $item->tgl_pendaftaran_ba_bongkar])['tgl_pendaftaran_ba_bongkar'];
            return $item;
        });
        // dd($bongkar);
        ...
        // dd($sbpData);
        return view('Pengawasanlain.ba-pengawasan-bongkar.index', compact('bongkar', 'pascapenindakan', 'sbpData', 'penyidikan'));
    }
    ...
    public function create(Request $request)
    {
        ...
        $laporan = TblLaporanInformasi::where('id_pra_penindakan', $sbpData->id_pra_penindakan_ref)->first();
        ...
        $tahunprint = !empty($tglprint) ? date('Y', strtotime($tglprint)) : '-';
        ...
        return view(
            'Pengawasanlain.ba-pengawasan-bongkar.create',
            compact(
                'users',
                'no_ref',
                'id_penyidikan',
                ...
                'laporan', // Menambahkan data TblLaporanInformasi
                'data',
            ),
        );
    }
    ...
    {
        TblBaPengawasanBongkar::create($request->except(['jenis_kemasan', 'jumlah_kemasan', 'dilaporkan', 'dibongkar', 'selisih', 'keterangan']));

        $no_ref = TblNoRef::first();
        $no_ref->no_pendaftaran_ba_bongkar += 1;
        $no_ref->save();

        return redirect()->route('ba-pengawasan-bongkar.index')->with('success', 'Data berhasil disimpan dan nomor referensi telah diperbarui.');
    }

    public function edit($id)
    {
        $bongkar = TblBaPengawasanBongkar::where('id', $id)->first();
        if (!$bongkar) {
            return redirect()->back()->with('error', 'Data tidak ditemukan.');
        }

        $penyidikan = TblPenyidikan::where('id_penyidikan', $bongkar->id_penyidikan_ref)->first();

        $pascapenindakan = TblPascaPenindakan::where('id_pasca_penindakan', $penyidikan->id_pasca_penindakan_ref)->first();

        $sbpData = TblSbp::with('laporanInformasi')->where('id_penindakan', $pascapenindakan->id_penindakan_ref)->first();

        $laporan = TblLaporanInformasi::where('id_pra_penindakan', $sbpData->pluck('id_pra_penindakan_ref'))->get();

        $users = User::all();

        $no_ref = TblNoRef::first();

        return view('Pengawasanlain.ba-pengawasan-bongkar.edit', compact('bongkar', 'users', 'no_ref', 'penyidikan', 'pascapenindakan', 'sbpData', 'laporan'));
    }

    public function update($id)
    {
        $data = request()->except(['jenis_kemasan', 'jumlah_kemasan', 'dilaporkan', 'dibongkar', 'selisih', 'keterangan']);

        $item = TblBaPengawasanBongkar::find($id);
        if ($item) {
            $item->update($data);
            return redirect()->route('ba-pengawasan-bongkar.index')->with('success', 'Data berhasil diperbarui.');
        }

        return redirect()->route('ba-pengawasan-bongkar.index')->with('error', 'Data tidak ditemukan.');
    }

    public function destroy($id)
    {
        $bongkar = TblBaPengawasanBongkar::find($id);
        if ($bongkar) {
            $bongkar->delete();
            return redirect()->route('ba-pengawasan-bongkar.index')->with('success', 'Data berhasil dihapus.');
        }
        return redirect()->route('ba-pengawasan-bongkar.index')->with('error', 'Data tidak ditemukan.');
    }

    public function print_ba_bongkar($id)
    {
        $bongkar = TblBaPengawasanBongkar::where('id', $id)->first();
        if (!$bongkar) {
        ...
        $tglBaSegelOriginal = $bongkar->tgl_pendaftaran_ba_bongkar ?? null;
        // Gabungkan data yang sudah ada dengan hasil formatDates
        $formattedData = $this->formatDates(array_diff_key($bongkar->toArray(), ['tgl_pendaftaran_ba_bongkar' => '']));
        $data = array_merge($data, $formattedData); // Gabungkan data, jangan timpa
        $data['tgl_pendaftaran_ba_bongkar'] = $tglBaSegelOriginal;
        $data['tgl_pendaftaran_ba_bongkarp'] = $this->formatDates(['tgl_pendaftaran_ba_bongkar' => $tglBaSegelOriginal])['tgl_pendaftaran_ba_bongkar'] ?? '-';

        $pejabatKeys = ['pejabat_pemeriksa_barang', 'pejabat_kedua_ctp'];
        ...
        foreach ($pejabatKeys as $key) {
            if ($key === 'id_pejabat_sp_2') {
                ...
                // Untuk pejabat lain, gunakan data dari bongkar
                $pejabat = $bongkar->pejabat($key)->first();
                $data[$key . '_nama'] = $pejabat->nama_admin ?? '-';
                $data[$key . '_pangkat'] = $pejabat->pangkat ?? '-';
                $data[$key . '_jabatan'] = $pejabat->jabatan ?? '-';
                $data[$key . '_nip'] = $pejabat->nip ?? '-';
        ...
        if (!empty($data['tgl_pendaftaran_ba_bongkar']) && $this->isValidDate($data['tgl_pendaftaran_ba_bongkar'])) {
            $tglBastInstansi = Carbon::parse($data['tgl_pendaftaran_ba_bongkar']);
        ...
        $tglctp = $bongkar->tgl_pendaftaran_ba_bongkar;
        ...
        $templateProcessor = new TemplateProcessor(resource_path('templates/Pengawasanlainnya/ba-pengawasan-bongkar/ba-pengawasan-bongkar.docx'));
        ...
        $fileName = "Dokumen_Pengawasan_Lain_Berita_Pengawasan_Bongkar_No_{$bongkar->no_pendaftaran_ba_bongkar}.docx";
    ...
    public function print_lpt_ba_bongkar($id)
    {
        $bongkar = TblBaPengawasanBongkar::where('id', $id)->first();
        if (!$bongkar) {
            return response()->json(['error' => 'Data tidak ditemukan.'], 404);
        ...
        $pejabatKeys = ['petugas_ba_bongkar_lpt', 'pejabat_kedua_ctp'];
        ...
        $templateProcessor = new TemplateProcessor(resource_path('templates/Pengawasanlainnya/ba-pengawasan-bongkar/laporan-pelaksanaan-tugas-ba-bongkar.docx'));
        foreach ($data as $key => $value) {
            if (!is_array($value)) {
                $templateProcessor->setValue($key, $value);
            }
        ...
        $fileName = "Dokumen_Pengawasan_Lain_Laporan_Pelaksanaan_Tugas_N0_{$bongkar->no_pendaftaran_ba_bongkar}.docx";
        $filePath = storage_path('app/public/' . $fileName);
        $templateProcessor->saveAs($filePath);
    ...
    public function print_lpp_ba_bongkar($id)
    {
        $bongkar = TblBaPengawasanBongkar::where('id', $id)->first();
        if (!$bongkar) {
            return response()->json(['error' => 'Data tidak ditemukan.'], 404);
        ...
        $tglBaSegelOriginal = $bongkar->tanggal_pengawasan_lpp_ba_bongkar ?? null;
        $tglKedatanganOriginal = $bongkar->tanggal_kedatangan_lpp_ba_bongkar ?? null;

        // Format seluruh data kecuali tanggal_pengawasan_lpp_ba_bongkar dan tanggal_kedatangan_lpp_ba_bongkar
        $formattedData = $this->formatDates(array_diff_key($bongkar->toArray(), ['tanggal_pengawasan_lpp_ba_bongkar' => '', 'tanggal_kedatangan_lpp_ba_bongkar' => '']));

        // Gabungkan data hasil formatDates ke $data
        $data = array_merge($data, $formattedData);

        // Simpan nilai original dan hasil format untuk tanggal_pengawasan
        $data['tanggal_pengawasan_lpp_ba_bongkar'] = $tglBaSegelOriginal;
        $data['tanggal_pengawasan_lpp_ba_bongkarr'] =
            $this->formatDates([
                'tanggal_pengawasan_lpp_ba_bongkar' => $tglBaSegelOriginal,
            ])['tanggal_pengawasan_lpp_ba_bongkar'] ?? '-';

        // Simpan nilai original dan hasil format untuk tanggal_kedatangan
        $data['tanggal_kedatangan_lpp_ba_bongkar'] = $tglKedatanganOriginal;
        $data['tanggal_kedatangan_lpp_ba_bongkarr'] =
            $this->formatDates([
                'tanggal_kedatangan_lpp_ba_bongkar' => $tglKedatanganOriginal,
            ])['tanggal_kedatangan_lpp_ba_bongkar'] ?? '-';

        $pejabatKeys = ['pejabat_mengawasi_bongkar', 'pejabat_kedua_ctp'];
        ...
        // $tglctp = $bongkar->tanggal_pengawasan_lpp_ba_bongkar;
        // $tahunctp = !empty($tglctp) ? date('Y', strtotime($tglctp)) : '-';
        // $data['tahun_ctp'] = $tahunctp;
        ...
        $templateProcessor = new TemplateProcessor(resource_path('templates/Pengawasanlainnya/ba-pengawasan-bongkar/laporan-pengawasan-bongkar.docx'));
        foreach ($data as $key => $value) {
            if (!is_array($value)) {
                $templateProcessor->setValue($key, $value);
            }
        ...
        $kemasanItems = TblKemasanBaBongkar::where('id_ba_bongkar', $bongkar->id_ba_bongkar)->get();

        // Persiapkan data kemasan untuk cloneRowAndSetValues
        $kemasanData = [];

        if ($kemasanItems->count() > 0) {
            foreach ($kemasanItems as $index => $item) {
                $kemasanData[] = [
                    'no' => $index + 1, // Menambahkan nomor urut dimulai dari 1
                    'jenis_kemasan' => $item->jenis_kemasan ?? '-',
                    'jumlah_kemasan' => $item->jumlah_kemasan ?? '-',
                    'dilaporkan' => $item->dilaporkan ?? '-',
                    'dibongkar' => $item->dibongkar ?? '-',
                    'selisih' => $item->selisih ?? '-',
                    'keterangan' => $item->keterangan ?? '-',
                ];
            }
        } else {
            // Jika tidak ada data kemasan, tambahkan satu baris kosong
            $kemasanData[] = [
                'no' => '-',
                'jenis_kemasan' => '-',
                'jumlah_kemasan' => '-',
        ...
        // Clone row dan set values untuk kemasan
        $templateProcessor->cloneRowAndSetValues('jenis_kemasan', $kemasanData);

        $fileName = "Dokumen_Pengawasan_Lain_Laporan_Pengawasan_N0_{$bongkar->no_pendaftaran_ba_bongkar}.docx";
        $filePath = storage_path('app/public/' . $fileName);
        $templateProcessor->saveAs($filePath);
