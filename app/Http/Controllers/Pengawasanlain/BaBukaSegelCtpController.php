...
class BaBukaSegelCtpController extends Controller
{
    ...
    {
        $bukasegelctp = TblBaBukaSegelCtp::select('id', 'tgl_ba_buka_segel_ctp', 'no_ba_buka_segel_ctp')->get();

        $bukasegelctp = $bukasegelctp->map(function ($item) {
            $item->tgl_ba_buka_segel_ctp = $this->formatDates(['tgl_ba_buka_segel_ctp' => $item->tgl_ba_buka_segel_ctp])['tgl_ba_buka_segel_ctp'];
            return $item;
        });
        // dd($bukasegelctp);
        ...
        // dd($sbpData);
        return view('Pengawasanlain.ba-buka-segel-ctp.index', compact('bukasegelctp', 'pascapenindakan', 'sbpData', 'penyidikan'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view(
            'Pengawasanlain.ba-buka-segel-ctp.create',
            compact(
                'users',
                'no_ref',
                'id_penyidikan',
    ...
    {
        TblBaBukaSegelCtp::create($request->all());
        $no_ref = TblNoRef::first();
        $no_ref->no_ba_buka_segel_ctp += 1;
        $no_ref->save();

        return redirect()->route('ba-buka-segel-ctp.index')->with('success', 'Data berhasil disimpan dan nomor referensi telah diperbarui.');
    }

    public function edit($id)
    {
        $bukasegelctp = TblBaBukaSegelCtp::where('id', $id)->first();
        if (!$bukasegelctp) {
            return redirect()->back()->with('error', 'Data tidak ditemukan.');
        }

        $penyidikan = TblPenyidikan::where('id_penyidikan', $bukasegelctp->id_penyidikan_ref)->first();
        ...
        return view('Pengawasanlain.ba-buka-segel-ctp.edit', compact('bukasegelctp', 'users', 'no_ref', 'penyidikan', 'pascapenindakan', 'sbpData', 'laporanInformasi'));
    }
    ...
    public function update($id)
    {
        ...
        $item = TblBaBukaSegelCtp::find($id);
        if ($item) {
            $item->update($data);
            return redirect()->route('ba-buka-segel-ctp.index')->with('success', 'Data berhasil diperbarui.');
        }

        return redirect()->route('ba-buka-segel-ctp.index')->with('error', 'Data tidak ditemukan.');
    }

    public function destroy($id)
    {
        $bukasegelctp = TblBaBukaSegelCtp::find($id);
        if ($bukasegelctp) {
            $bukasegelctp->delete();
            return redirect()->route('ba-buka-segel-ctp.index')->with('success', 'Data berhasil dihapus.');
        }
        return redirect()->route('ba-buka-segel-ctp.index')->with('error', 'Data tidak ditemukan.');
    }

    public function print_ba_buka_segel_ctp($id)
    {
        $bukasegelctp = TblBaBukaSegelCtp::where('id', $id)->first();
        if (!$bukasegelctp) {
        ...
        $tglBaSegelOriginal = $bukasegelctp->tgl_ba_buka_segel_ctp ?? null;
        // Gabungkan data yang sudah ada dengan hasil formatDates
        $formattedData = $this->formatDates(array_diff_key($bukasegelctp->toArray(), ['tgl_ba_buka_segel_ctp' => '']));
        $data = array_merge($data, $formattedData); // Gabungkan data, jangan timpa
        $data['tgl_ba_buka_segel_ctp'] = $tglBaSegelOriginal;
        $data['tgl_ba_buka_segel_ctpp'] = $this->formatDates(['tgl_ba_buka_segel_ctp' => $tglBaSegelOriginal])['tgl_ba_buka_segel_ctp'] ?? '-';

        $pejabatKeys = ['pejabat_pertama_buka_ctp', 'pejabat_kedua_buka_ctp', 'id_pejabat_sp_2'];
        ...
        foreach ($pejabatKeys as $key) {
            ...
            } elseif ($bukasegelctp->$key) {
                // Untuk pejabat lain, gunakan data dari segelctp
                $pejabat = $bukasegelctp->pejabat($key)->first();
                $data[$key . '_nama'] = $pejabat->nama_admin ?? '-';
                $data[$key . '_pangkat'] = $pejabat->pangkat ?? '-';
                $data[$key . '_jabatan'] = $pejabat->jabatan ?? '-';
                $data[$key . '_nip'] = $pejabat->nip ?? '-';
        ...
        if (!empty($data['tgl_ba_buka_segel_ctp']) && $this->isValidDate($data['tgl_ba_buka_segel_ctp'])) {
            $tglBastInstansi = Carbon::parse($data['tgl_ba_buka_segel_ctp']);
            ...
            $data['formatBaBukaCtp'] = "$namaHari tanggal $tanggal bulan $bulan tahun $tahun";
        } else {
            $data['formatBaBukaCtp'] = '';
        }

        $tglctp = $bukasegelctp->tgl_ba_buka_segel_ctp;
        $tahunctp = !empty($tglctp) ? date('Y', strtotime($tglctp)) : '-';
        $data['tahun_buka_ctp'] = $tahunctp;
        ...
        $templateProcessor = new TemplateProcessor(resource_path('templates/Pengawasanlainnya/ba-buka-segel-ctp.docx'));
        ...
        $fileName = "Dokumen_Pengawasan_Lain_Berita_Acara_Buka_Segel_CTP_No_{$bukasegelctp->no_ba_buka_segel_ctp}.docx";
