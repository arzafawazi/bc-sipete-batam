@extends('layouts.vertical', ['title' => 'B.A Segel CTP'])
    ...
    <div class="container-fluid">
        ...
        <div class="card mb-3 mt-4">
            <div class="card-header">
                <h5 class="card-title mb-0">Berita Acara Segel CTP</h5>
            </div>
            <div class="card-body">
                <form method="GET">
                    <div class="row">
                        ...
                        <table id="fixed-header-datatable" class="table table-hover align-middle border-separate"
                            ...
                            <thead>
                                <tr class="bg-light">
                                    ...
                                    <th class="px-3 text-center py-3" style="width: 10%">No BA Segel CTP</th>
                                    <th class="px-3 text-center py-3" style="width: 15%">Tanggal Ba Segel CTP</th>
                                    <th class="text-center px-3 py-3" style="width: 20%">Opsi</th>
                                </tr>
                            </thead>
                            <tbody align="center">
                                @foreach ($segelctp as $index => $segelctp)
                                    <tr class="shadow-sm">
                                        <td class="text-center fw-medium">{{ $index + 1 }}.</td>
                                        <td class="fw-medium">{{ $segelctp->no_ba_segel_ctp }}</td>
                                        <td class="fw-medium">{{ $segelctp->tgl_ba_segel_ctp }}</td>
                                        <td>
                                            <div class="d-flex gap-1 justify-content-center">
                                                <a href="{{ route('ba-segel-ctp.edit', ['ba_segel_ctp' => $segelctp->id]) }}"
                                                    class="btn btn-soft-success btn-icon btn-sm rounded-pill">
                                                    <i data-feather="edit" class="icon-sm"></i> Edit
                                                </a>
                                                <form action="{{ route('ba-segel-ctp.destroy', $segelctp->id) }}"
                                                ...
                                                <a href="{{ route('ba-segel-ctp.print', $segelctp->id) }}"
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('pilih-laporan')) {
                    ...
                    let url =
                        `/Pengawasanlain/ba-segel-ctp/create?id_penyidikan=${encodeURIComponent(nomorLaporan)}`;
